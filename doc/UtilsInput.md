...
| `GetCh()` | `int` | 1 文字を読み取って文字コードを返す（ブロッキング）。失敗時は `-1` を返す。エコーなし・行バッファなし |
...
`GetCh()` は入力時に端末設定を一時的に変更し、読み取り後に元へ戻します。
...
- Unix 系の `GetCh()` は端末設定を一時的に変更します。
  シグナルで強制終了した場合、ターミナルの設定が元に戻らない可能性があります。
- Unix 系の `GetCh()` は `tcgetattr` / `tcsetattr` / `read` の失敗時に `-1` を返し、失敗した syscalls の `errno` を保ちます。
- `KbHit()` の挙動は OS により異なります。  
  Windows ではキー入力の有無を確認するだけで文字を消費しません。Unix 系でも `KbHit()` の直後に `GetCh()` を呼ぶことで、検出した文字をそのまま取得できます。
