...
> **⚠️ Unix 系の既知の問題**: 現在の Unix 実装では `KbHit()` が `read()` で 1 文字を消費するため、直後の `GetCh()` は別の入力を読む可能性があります（実装修正予定）。このサンプルは Windows では正しく動作します。
...
> **⚠️ Unix 系の既知の問題**: 現在の Unix 実装では `KbHit()` が入力を消費するため、`GetCh()` で読む文字が期待したキーと異なる場合があります（実装修正予定）。このサンプルは Windows では正しく動作します。
...
> **⚠️ Unix 系の既知の問題**: 現在の Unix 実装では `KbHit()` が入力を消費するため、`GetCh()` で読む文字がスペースや `q` でなく別の文字になる場合があります（実装修正予定）。このサンプルは Windows では正しく動作します。
...
- `KbHit()` の挙動は OS により異なります。  
  Windows ではキー入力の有無を確認するだけで文字を消費しませんが、現在の Unix 系実装では `KbHit()` 呼び出し時に入力を 1 文字消費します。  
  そのため Unix 系では `KbHit()` の直後に `GetCh()` を呼ぶと、`KbHit()` で検出した文字をそのまま読めない場合があります。
